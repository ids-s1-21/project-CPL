...
<small>* The top 5 kinds of sports that cause this spike</small>
...
olympics_model_medal <- olympics_model %>%
  group_by(height) %>%
  mutate(probability_medal = sum(medal_yes)/nrow(olympics_model)) %>%
  filter(medal_yes == 1) %>%
  filter(height >= 150, height <= 185)

olympics_model_medal %>%
