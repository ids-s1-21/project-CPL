...
## We are going to add variable(sports) and analyse questions by sports {data-transition="zoom"}
...
3. Make a model related to the age and the probability of wining medals.
...
## What kind of sports that younger athletes are good at? {data-transition="zoom"}

```{r Top12-sports-that-younger-athletes-are-good-at , echo = FALSE}
olympics %>%
  group_by(sport) %>%
  na.omit(medal) %>%
  mutate(n_medal = medal %in% c("Bronze", "Silver", "Gold")) %>%
...
:::: column
```{r All-sports-that-younger-athletes-are-good-at , echo = FALSE}
olympics %>%
  group_by(sport) %>%
  na.omit(medal) %>%
  mutate(n_medal = medal %in% c("Bronze", "Silver", "Gold")) %>%
...
## The two spikes around the age 46 and 52. {data-transition="zoom"}
...
## reveal.js {data-transition="zoom"}
